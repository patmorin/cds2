...
\begin{proof}
  ...
  \begin{compactenum}
    ...
    \hussein{do we mean
    $B(H^*) \subseteq B(H)$.} \pat{I think $B(H^\star)=B(H)$, no?} \hussein{Here if $v$ is not a vertex cut like the right most picture, then $v$ is deleted for ever. Right?}
  \end{compactenum}
\end{proof}
...
\begin{proof}
  ...
  \end{figure}
  \begin{enumerate}[(i)]
    \item $\deg^+_H(z)=1$ for some $z\in N_H(w)$. By \cref{inner_degree_1}, $H[N_H[z]]$ is isomorphic to $K_4$.  Let $u$ and $v$ be the two vertices, other than $z$ on the outer face of $H[N_H[z]]$.  Since $\deg_{H-B(H)}(w)=0$, \cref{useful_little_guy} implies that $w$ is the only vertex of $H$ in the interior of the cycle $uvz$. Let $H'$ be a dom-minimal graph that dom-respects $H-v$.  Since $\deg^+_{H-v}(w)=\deg^+_{H-v}(z)=0$, neither $x$ or $w$ are vertices of $H'$. Therefore, $|H'|\le |H-\{u,v,w\}|=|H|-3$.  By \cref{boundary_subset}, $B(H')\subseteq N^+_H(v)\cup B(H-\{v,z,w\})$, so $|B(H')|\le |B(H)|+2-3=|B(H)|-1$, which satisfies the conditions of the lemma.
...
\begin{compactenum}[{[}bg1{]}]
    ...
    \item Otherwise, if there exists distinct $u,w\in B(G_i)$ and $w\in B(G_i-B(G_i))$ such that $\deg^+_{G_i}(v)=2$, $\deg^+_{G_i-v}(w)\ge 3$, and $N^+_{G_i}(u)\subseteq N_{G_i}(w)$ then set $\Delta_i:=\{v,w\}$.
...
\pat{I'm working here...}

\begin{proof}
    There are three main tasks required to implement the algorithm:
    \begin{compactenum}
        ...
        \item Identify when $G_i$ is $2$-critical and find a set $\Delta_{r-1}$ of size at most $2(|B(G_i-B(G_i))| + |I(G_i-B(G_i))|)$ that dominates $I(G_i)$.
    \end{compactenum}

    The main data structuring tool used to accomplish these steps efficiently is a technique for storing sorted counters.  Each vertex $v$ of $G$ maintains a counter $\delta_i(v):=|N_G(v)\cap I(G_i)|$.  The vertices in $B(G_i)$ are kept in a doubly list of lists $\beta$.  Each item in $\beta$ is a doubly-linked list, called a \defin{bucket} that stores a non-empty set $\beta_d:=\{v\in B(G_i):\delta(v)=d\}$ for some specific value of $d$. Then $\beta$ itself is a doubly-linked list that stores the buckets by increasing value of $d$.  A similar structure $\zeta$ is used to store the vertices of $B(G_i-B(G_i))$.  Since the value of $\delta_i(v)$ decreases monotonically as $i$ increases, $\sum_{v\in V(G)}\delta_0(v)=3n-6$, and each vertex enters and leaves each of $\beta$ and $\zeta$ at most once, it is straightforward to maintain $\beta$ and $\zeta$ so that all operations on them take a total of $O(n)$ time over the entire execution of the algorithm.  From this point on, we will no longer discuss the maintenance of these lists, but we will use them to identify the vertex $v_i$, or the vertices $u,v,w$ when needed.

    \paragraph{Identifying $\Delta_i$:}
    To identify the set $\Delta_i$ use $\beta$ to find the vertex $v_i\in B(G_i)$ that maximizes $\deg^+_{G_i}(v_i)$.  If $\deg^+_{G_i}(v_i)\ge 3$ then \cref{bg_high_degree} applies and there is nothing more to do.   Otherwise, we use $\zeta$ to identify the vertex $w\in B(G_i-B(G_i))$ that maximizes $\deg_{G_i-B(G_i)}(w)$.  If $\deg_{G_i-B(G_i)}(w)\ge 2$, then this vertex can be used as the vertex $w$ in \cref{bg_two_three}.  In this case, the vertices $u$ and $v$ can be found in $N_{G_i}(w)$ in $O(\deg_G(v))$ time.  If $\deg_{G_i-B(G_i)}(w)=1$ then \cref{bg_leaf} applies and the vertex $v_i\in N_{G_i}(w)$ can be found in $O(\deg_G(w))$ time (this is the vertex $v_1$ in \cref{killing_a_leaf}).  If $\deg_{G_i-B(G_i)}(w)=0$ then \cref{bg_degree_zero} applies.  The vertex $v_i\in N_{G_i}(w)$ can be found in $O(\deg_G(w))$ time (this is the vertex $v$ in \cref{isolated_fig}).
    
\hussein{I am not sure, if it is straightforward to do the updates on the data structure!, the insertion in $\beta$ is not $O(1)$, it might be $\log n$.}

\pat{The insertion of $v$ into $\beta$ can be done in $O(\deg_G(v))$ time by starting at the small end of the list and stopping when we get to the correct bucket.}
    
    \paragraph{Computing $G_{i+1}$:}  After computing $\Delta_i$, we must compute a dom-minimal graph $G_{i+1}$ that dom-respects $G_i-\Delta_i$.  Since $G_i$ was dom-minimal, the only violations of dom-minimality occur at vertices incident to vertices in $N^+_{G_i}(\Delta_i)$ and at edges incident to faces with a vertex in $N^+_{G_i}(\Delta_i)$.  In particular, violations of \cref{bad_vertex,inner_degree_1} can be detected when adjusting the counters of vertices adjacent to vertices in $N^+_{G_i}(\Delta_i)$.  Violations of \cref{bad_edge} can be detected by examining the inner faces incident to each vertex in $N^+_{G_i}(\Delta_i)$. 

    \paragraph{Handling the $2$-critical case:}
    When none of \cref{bg_high_degree,bg_two_three,bg_degree_zero,bg_leaf} apply, $G_i$ is $2$-critical.  In this case, 

    
    % In each of these cases, we remove a vertex $v_i$ from $G_i$ and then find a dom-minimal graph $G_{i+1}$ that dom-respects $G_i-v_i$.  Removing $v_i$ from $G_i$ is easy.  If $G_i-v_i$ is not already dom-minimal, then any violation of dom-minimality involves a vertex in $N_{G_i}(v_i)$.  Indeed, any violation of dom-minimality occurs because some vertex $x$ in $I(G_i)$ is in $B(G_i-v_i)$.  This causes the inner-degree of some vertex $v\in B(G_i)$ to drop below $2$ or causes some edge $vw$ on the outer face to no longer bound a triangle $vwx$ with $x$ being an inner vertex. $x$ of some triangular face $vwx$


    % This allows us to find the vertex $v_i\in B(G_i)$ that maximizes $\deg^+_{G_i}(v_i)$ in constant time.


    % If $\deg^+_{G_i}(v_i)\ge 3$ then we remove it. Removing $v_i$ causes some counters of vertices in $N_{G_i}(v_i)$ to decrease and causes some vertices in $N_{G_i}(v_i)$ to appear in $B(G_i-v_i)$ for the first time.  Since all of these counters decrease monotonically and a new vertex that appears on $B(G_i-v_i)$ remains there until its indegree becomes zero, the total time spent managing these counters is $O(n)$.
    ...
    % \begin{compactenum}
    %     \item Data structure that stores vertices in $B(G_i)$ ordered by inner-degree lets us find vertices of degree at least $3$ in constant time.

    %     \item Data structure that stores $B(G_i-B(G_i))$ ordered by inner-degree in $G_i-B(G_i)$ lets us find the vertex $w$ in \cref{2_3_figure} in constant time.

    %     \item The preceding structure also signals us when \cref{leaf_killer} and \cref{degree_zero_killer} are applicable.

    %     \item Algorithms for implementing \cref{leaf_killer} (trivial) and \cref{degree_zero_killer} (trickier, because of special choice of $w$ and $uv$).

    %     \item All of these need algorithms for dom-minimal reductions.
    % \end{compactenum}
\end{proof}
