...
% \begin{proof}
%   \pat{There is still some work to do here.  We should break this up into cases where we eliminate bridges and cut vertices first.  Then, finally, we're left with the case where each component of $H-B(H)$ is a $2$-connected critical graph. To deal with this, we first deal with the stupid cases where this graph is just a $3$-cycle or a cycle with one inner vertex. (We can handle these by choosing any vertex on the face).  Otherwise the graph is an outerplanar graph with stars in some of its faces.  See \cref{contraction}. In each such face we'll contract one of the star edges (we should choose one incident to a chord of $H[B(H)]$, which is why we had to rule out the second stupid case) to we get an outerplanar graph and those special faces are star-triangulated.  When we contract, the vertex we contract into gets a weight of $3$. All other vertices get a weight of $2$. The total weight is then $2|B(H-B(H))|+|H-BB(H)|\le |H|$, by \cref{big_boundary}. Then we $3$-colour and take the lowest weight colour class.  The key point is that, for each weight-$2$ vertex $w$ we choose, we have to include $w$ and a neighbour $v$ of $w$ in $B(H)$.  For each weight $3$ vertex $w$ we choose, we have to take $w$, a neighbour $v$ of $w$ in $B(H)$, and the inner vertex of $H-B(H)$ that caused $w$ to have weight $3$.}
% \begin{figure}
%     \begin{center}
%         \begin{tabular}{ccc}
%             \includegraphics[page=1]{figs/contractor} &
%             \includegraphics[page=2]{figs/contractor} &
%             \includegraphics[page=3]{figs/contractor}
%         \end{tabular}
%         \caption{Contracting an inner vertex of a $1$-planar graph into a boundary vertex}
%         \label{contraction}
%     \end{center}
% \end{figure}
% \end{proof}
...
\Cref{boundary_vs_walk,walk_vs_leaves} immediately give the following corollary:
...
\begin{lem}
  Let $H$ be a $2$-connected critical generalized near-triangulation with at least $3$ vertices.  Then there exists $X_0,X_1,X_2\subseteq V(H)$ such that
  \begin{compactenum}[(i)]
    \item $\sum_{i=0}^2(2|X_i\cap B(H)|+|X_i\cap I(H)| \le 2|B(H)|+I(H)$; \label[p]{total_size_critical}
    \item for each $i\in\{0,1,2\}$, $X_i$ dominates $H$; and \label[p]{dominates_h}
    \item for each $i\in\{0,1,2\}$, each component of $H[X_i]$ contains at least one vertex in $B(H)$. \label[p]{connectivity_critical}
  \end{compactenum}
\end{lem}

\begin{proof}
  If $H$ has three vertices then choose any vertex $v$ of $H$ and set $X_0:=X_1:=X_2=\{v\}$.  If $H$ has four vertices, then $B(H)$ contains a vertex $v$ of degree $3$, in which case we take $X_0:=X_1:=X_2=\{v\}$. \pat{Split into two cases, giving a partition in each case.} In each of these cases, the choice of $X_0$, $X_1$, and $X_2$ easily satisfy \cref{total_size_critical,dominates_h,connectivity_critical}.

  We now assume that $H$ has at least five vertices.  If $H[B(H)]$ has a single inner face $f$ then, by \cref{critical_structure}, $I(H)$ consists of a single vertex $x$ in the interior of $f$ that is adjacent to every vertex in $f$. In this case we choose a vertex $v$ of $f$ and set $X_0:=X_1:=X_2:=\{v,x\}$.  Then $2|X_0\cap B(H)|+|X_0\cap I(H)| = 3 = 9/3 = (8 + 1) \le (2|B(H)|+|I(H)|)/3$. Therefore, this choice of $X_0$, $X_1$, and $X_2$ satisfy \cref{total_size_critical}, since $X_0=X_1=X_2$.

  We now assume that $H$ has at least five vertices and that $H[B(H)]$ has at least two inner faces.  In this case, we will prove a strengthening of the lemma in which
  \begin{inparaenum}[(a)]
    \item $X_0$, $X_1$, and $X_2$ are pairwise-disjoint, \label[p]{disjoint}
    \item $X_0$, $X_1$, and $X_2$ cover $B(H)$, and \label[p]{cover}
    \item no edge of $H[B(H)]$ has both endpoints in a single $X_i$.\label[p]{proper}
  \end{inparaenum}
  Note that \cref{disjoint} immediately implies that these sets satisfy \cref{total_size_critical}, so we will ignore \cref{total_size_critical} from this point on.
  % \Cref{critical_structure} and \cref{} the fact that $X_0$, $X_1$, and $X_2$ cover $B(H)$ immediately imply \cref{dominates_h,connectivity_critical}.  Thus, we will ignore \cref{total_size_critical,dominates_h,connectivity_critical} from this point onward and focus on


   % so we will ignore \cref{total_size_critical} from this point onward.

  We prove this stronger lemma by induction on $|I(H)|$.  If $|I(H)|=0$ then $H$ is an edge-maximal outerplanar graph, and therefore has a proper $3$-colouring.  We take $X_0$, $X_1$, and $X_2$ to be the three colour classes in this colouring.  This choice clearly satisfies \cref{disjoint,cover,proper}.
  % Since $V(H)=B(H)$ and $\{X_0,X_1,X_2\}$ is partition of $B(H)$, this choice clearly satisfies \cref{total_size_critical}.
  Since each vertex of $H$ is included in at least one triangle, each vertex of $H$ is dominated by each of $X_0$, $X_1$, and $X_2$, so this choice satisfies \cref{dominates_h}.  Since $V(H)=B(H)$ this choice trivially satisfies \cref{connectivity_critical}.

  Otherwise $|I(H)|\ge 1$. Let $x$ be an inner vertex of $H$ and let $f$ be the face of $H[B(H)]$ that contains $x$. If $f$ has only three vertices, then we apply induction on $H-x$. Again, the resulting sets $X_0$, $X_1$, and $X_2$ clearly satisfy \cref{disjoint,cover,proper}.  By \cref{proper}, the face $f$ has one vertex in each of $X_0$, $X_1$, and $X_2$, so each of these sets dominates $H$, so these sets satisfy \cref{dominates_h}. These sets satisfy \cref{connectivity_critical} by the inductive hypothesis.

  Since $H[B(H)]$ has at least two inner faces and $H$ is biconnected, $f$ contains an edge $vw$ that is on the boundary of two faces of $H$.  Let $H'$ be the graph obtained by contracting the edge $vx$ to obtain a single vertex $v'$. Then $H'$ is still biconnected and $H'[B(H')]$ still has at least two inner faces. \pat{Continue here....}
\end{proof}


\begin{lem}
  Let $H$ be a $2$-critical generalized near-triangulation \pat{Add condition on the size of $H-B(H)$}.  Then there exists $X_0,X_1,X_2\subseteq V(H)$ such that
  \begin{compactenum}[(i)]
