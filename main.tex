...
A \defin{corner} of a face $f$ in a plane graph $H$ is an ordered pair $(uv,vw)$ of edges in $H$ that occur consecutively around $v$ and such that the face $f$ is to the left of the path $uvw$.  Note that this includes the possibility that $u=w$, which occurs when $v$ has degree $1$.  We let $\corners(H)$ denote the set of corners on the outer face of $H$.

\begin{lem}\label{corner_degree}
  Let $H$ be a $2$-critical generalized near-triangulation, and let $C$ be a component of $H-B(H)$ with at least two vertices. Then $|E_H(V(C),B(H))|\ge 2|\corners(C)|$.
\end{lem}

\begin{proof}
  Consider some corner $(uv, vw)$ of the outer face of $C$.  Since $u,v,w$ are in $I(H)$, $H$ contains faces $uxv$ and $vyw$ that are to the left of $uv$ and $vw$, respectively. Since $uvw$ is in the boundary walk of the outer face of $C$ $C$, these faces are not faces of $H-B(H)$, so $x,y\in B(H)$.  We claim that $x\neq y$.  To see this, there are two cases to consider.
  \begin{compactenum}
    ...
    \item If $u=w$ and $x=y$, then there are two inner faces of $H$ that have $uv$ on their boundary, which is not possible because $H$ does not have parallel edges.
  \end{compactenum}
  Therefore, the edges $vx$ and $vy$ can be assigned to the corner $(uv,vw)$. By doing this for each corner of the outer face of $C$ we obtain a mapping from a subset of $E_H(V(C),B(H))$ onto $\corners(C)$ in which each corner has two edges mapped onto it. Therefure, $|E_H(V(C),B(H))|\ge 2|\corners(C)$.
\end{proof}
...
\begin{lem}
  Let $H$ be a $2$-critical generalized near-triangulation, and let $C$ be a component of $H-B(H)$. Then $|E_H(V(C),B(H))|\ge 2|B(C)|$.  Furthermore, if $C$ has at least three vertices and is not a star then $|E_H(V(C),B(H))|\ge 2(|B(C)| + |V_1(C)|)$
\end{lem}

\begin{proof}
  If $C$ is single vertex then $|E_H(V(C),B(H))|\ge 3 = 3|B(C)|\ge 2|B(C)|$. Otherwise, each vertex in $B(C)$ is the internal vertex of at least one corner in $\corners(C)$ so, by \cref{corner_degree}, $|E_H(V(C),B(H))|\ge 2|B(C)|$.  This establishes the first part of the corollary.
...
For each integer $r\ge 3$, let $W_r$ be the near-triangulation whose outer face is bounded by a cycle $v_0,\ldots,v_{r-1}$ that contains a single vertex $x$ in its interior and that is adjacent to each of $v_0,\ldots,v_{r-1}$.
...
\begin{lem}\label{two_connected_critical}
  ...
  \begin{compactenum}[(i)]
    ...
    \item For each inner face $f$ of $H[B(H)]$ and each $i\in\{0,1,2\}$, $V(f)\cap X_i\neq\emptyset$; and \label[p]{colourful} \pat{Consider eliminating this, since we only use it to prove \cref{dominates_h}.}
...
\begin{proof}
  If $H$ is isomorphic to $W_k$ for some odd integer $k\ge 3$, then we take $X_0:=\{v_0, x\}$, $X_1:=\{v_{2i-1}:i\in\{1,\ldots,\lfloor k/2\rfloor\}$, and $X_2:=\{v_{2i}:i\in\{1,\ldots,\lfloor k/2\rfloor\}$.  It is straightforward to verify that these sets satisfy \cref{proper,colourful,dominates_h}.  (The fact that $k$ is odd ensures that $v_0$ has a neighbour $v_1\in X_1$ and $v_{k-1}\in X_2$, which ensures \cref{dominates_h}---this is not true for even $k$.)   We now assume that $H$ is not isomorphic to $W_k$ for any integer $k$.  By \cref{critical_structure}, this implies that $H$ is outerplanar or that $H[B(H)]$ has at least two inner faces.

  We now proceed by induction on $|I(H)|$.  If $|I(H)|=0$ then $H$ is an edge-maximal outerplanar graph, and therefore has a proper $3$-colouring.  We take $X_0$, $X_1$, and $X_2$ to be the three colour classes in this colouring.  This choice clearly satisfies \cref{proper} and trivially satisfies \cref{colourful}.  Since each vertex of $H$ is included in at least one triangle, each vertex of $H$ is dominated by each of $X_0$, $X_1$, and $X_2$, so this choice satisfies \cref{dominates_h}.
...
\begin{proof}
  Label the vertices of $W_k$ as $v_0,\ldots,v_{k-1}$ so that $v=v_0$.  Then the sets $X_0:=\{v_0, x\}$, $X_1:=\{v_{2i-1}:i\in\{1,\ldots,\lfloor k/2\rfloor\}$, and $X_2:=\{v_{2i}:i\in\{1,\ldots,\lfloor k/2\rfloor\}$ satisfy the requirements of the lemma.
\end{proof}
...
\begin{lem}\label{two_connected_critical}
  Let $H$ be a connected critical generalized near-triangulation with at least $3$ vertices and not isomorphic to $W_k$ for any even integer $k$ and let $L$ be the set of degree-$1$ vertices of $H$.  Then there exists a partition $\{X_0,X_1,X_2\}$ of $V(H)$ and partition $\{L_0,L_1,L_2\}$ of $L$ such that
  \begin{compactenum}[(i)]
...
\begin{proof}
  ...
  If $L\neq\emptyset$ then let $z_0$ be a vertex in $L$ and let $z_0,\ldots,z_r$ the maximal path in $H$ such that each of $z_1,\ldots,z_{r-1}$ has degree $2$ in $H$. Consider the graph $H':=H-\{z_0,\ldots,z_{r-1}$.
  \begin{enumerate}
    \item If $z_r\in L$, then $H'$ is the graph that contains only $z_r$ and $H=z_0,\ldots,z_r$ is a path.  In this case $L:=\{z_0,z_r\}$. We take $X_i:=\{z_j:j\equiv i\pmod 3\}$ for each $i\in\{0,1,2\}$, $L_2=\{z_0\}$.  Starting with $L_0:=L_1:=\emptyset$ and $L_2:=\{z_0\}$, we then set
    $L_{(r-2)\bmod 3}\gets L_{(r-2)\bmod 3} \cup \{z_r\}$.  This produces sets that satisfy the requirements of the lemma.

    \item If $H'$ is isomorphic to $W_k$ for some even integer $k$ then we apply \cref{wheelie} with $v=z_r$ to obtain sets $X_0'$, $X_1'$, $X_2'$, $L_0'$, $L_1'$, and $L_2'$.

    \item Otherwise, we apply the currently lemma inductively on $H'$ to obtain sets $X_0'$, $X_1'$, $X_2'$, $L_0'$, $L_1'$, and $L_2'$.
  \end{enumerate}
  In the latter two cases, we may assume, without loss of generality (by renaming colours), that $z_r\in X_{r\bmod 3}'$.   Since $z_r\not\in L$, $\deg_H(z_r)\ge 3$. By \cref{wheelie} or the inductive hypothesis, $z_r$ has a neighbour $w$ in $H'$ that, without loss of generality (by renaming colours) is in $X_{(r+1)\bmod 3}$.

  Then we take $X_i:=X_{i}'\cup\{z_j:j\equiv i\pmod 3\}$, $L_0:=L_0'$, $L_1:= L_1'$, and $L_2:=L_2'\cup\{z_0\}$.  Then these sets satisfy the requirements of the lemma.  Indeed, $z_r$ has a neighbour $z_{r-1}$ in $X_{(r-1)\bmod 3}$, $z_r$ is in $X_{r\bmod 3}$ and $z_r$ has a neighbour $w$ in $X_{(r+1)\bmod 3}$.  Thus each $X_i$ dominates $z_r$.\pat{Explain further}
\end{proof}
...
\begin{proof}
  Let $B_0:=B(H)$, let $H_1:=H-B(H)$, let $B_1:=B(H_1)$, let $I_1:=I(H_1)$,  let $V_1:=V_1(H_1)$ and let $V_0:=V_0(H_1)$, let $C_2$ be the set of components of $H_1$ having exactly two vertices, and let $C_\star$ be the set of components of $H_1$ that are stars.
  ...
  For each biconnected component $C$ of $H-B(H)$ with at least $3$ vertices that is not a star, we apply \cref{two_connected_critical} to obtain sets $X_0,X_1,X_2,L_0,L_1,L_2$.
  For each $w\in L_j$ then take a neighbour $w'$ of $w$ in $B_0$ and add it to $X_j$.  This ensures that $w$ is dominated by $X_j$.  The total number of vertices added to $X_0$, $X_1$, and $X_2$ by this step is $|C|+|B(C)|+|L|=|C|+|B(C)|+|V_0(C)|$.

  For each degree-$0$ vertex $w$ in $H-B(H)$ we will choose a vertex in $N_H(v)\cap B(H)$ and place $v$ in each of $X_0$, $X_1$, and $X_2$. This accounts for a total of $3|V_0|$ vertices in $X_0$, $X_1$, and $X_2$.

  For each component $C$ in $C_2$ we choose one vertex $v$ in $B(H)$ that dominates $V(C)$ and place $v$ in each of $X_0$, $X_1$, and $X_2$. This accounts for a total of $3||$ vertices in $X_0$, $X_1$, and $X_2$.

  For each component $C$ in $C_{\star}$ we choose the vertex $w\in C$ that dominates the star $C$ and a vertex $v\in B(H)$ adjacent to $w$.  We place $\{v,w\}$ in each of $X_0$, $X_1$, and $X_2$.

  \pat{Not quite so simple. In the preceding three steps we need to make sure that we pick a vertex $v$ in $B(H)$ with $\deg^+_C(v)=2$ or that we use the same vertex twice if it is adjacent to more than one component. (We may need to do this for components of sizes $1$ and $2$, where vertices with $\deg^+_H(C)=1$ are unavoidable.)}

  % We call the vertices in $L$ \defin{leaves}.

  % Intuitively, each vertex in $I_1$ should appear in at most one $X_i$. Each vertex $w$ in $B_1$ appears in one $X_i$ and, in order to satisfy \cref{connectivity}, a neighbour $v\in B_0$ of $w$ should appear in the same $X_i$.  Each vertex $w$ in $L$ (which is also in $B_1$) may need to contribute an additional vertex in order to satisfy \cref{dominates_i}.
\end{proof}
