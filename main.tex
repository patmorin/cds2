...
\begin{compactenum}[({DM}1)]
    ...
    \item each edge $vw$ on the boundary of the outer face of $H$ is on the boundary an inner face $vwx$ of $H$ for some $x\in I(H)$. \label[dm]{bad_edge} \saeed{this sentence looks unclear }
\end{compactenum}
...
\pat{Discuss how current approach might be improved with further analysis, but something more global is needed to get $n/3$.}

\pat{Conjectures: $n/3$ for triangulations and $n/4$ for $4$-connected triangulations} \saeed{There are 4-connected triangulations with smallest connected dominating set of size $n/3$.}
