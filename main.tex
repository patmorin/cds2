...
For a graph $G$, let $|G|=|V(G)|$ denote the number of vertices of $G$.  A \defin{bridge} in a graph $G$ is an edge $e$ of $G$ such that $G-e$ has more connected components than $G$.  A set $X\subseteq V(G)$ \defin{dominates} a set $B\subseteq V(G)$ if each vertex in $B$ is in $X$ or adjacent to a vertex in $X$.  Thus, $X$ is a dominating set of $G$ if and only if $X$ dominates $V(G)$.
...
\begin{compactenum}[({DM}1)]
    ...
    \item each edge $vw$ on the boundary of the outer face of $G$ is on the boundary an inner face $vwx$ of $H$ for some $x\in I(H)$. \label{bad_edge}
\end{compactenum}
