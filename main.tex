...
\begin{proof}
  ...
  If $L\neq\emptyset$ then let $z$ be a vertex in $L$ and let $z_r,\ldots,z_0$ the maximal path in $H$ such that $z=z_r$ and each of $z_1,\ldots,z_{r-1}$ has degree $2$ in $H$. Consider the graph $H':=H-\{z_0,\ldots,z_{r-1}\}$.
  \begin{enumerate}
...
\begin{proof}
  Let $B_0:=B(H)$, let $H_1:=H-B(H)$, let $B_1:=B(H_1)$, let $I_1:=I(H_1)$, let $C_1$ be the set of components of $H_1$ having exactly one vertex, let $C_2$ be the set of components of $H_1$ having exactly two vertices, let $C_\star$ be the set of components of $H_1$ that are stars, let $C_{\boxtimes}$ be the set of components of $H_1$ that are even wheels, and let $\overline{C}$ be the set of componets of $H_1$ that are not in $C_1\cup C_2\cup C_{\star}\cup C_{\boxplus}$.
  ...
  For each component $C$ in $C_2$ we choose one vertex $v$ in $B(H)$ that dominates $V(C)$ and place $v$ in each of $X_0$, $X_1$, and $X_2$. This contributes a total of $3|C_2|$ vertices in $X_0$, $X_1$, and $X_2$. On the other hand, $\sum_{C\in C_2} |C|=2|C_2|$ and, by \cref{degree_corners_and_leaves},  $\sum_{C\in C_2} |\widehat{N}_H(C)|\ge 2|C_2|$.

  For each component $C$ in $C_{\star}$ we choose the vertex $w\in C$ that dominates the star $C$ and a vertex $v\in B(H)$ adjacent to $w$.  We place $\{v,w\}$ in each of $X_0$, $X_1$, and $X_2$.  This contributes a total of $6|C_{\star}|$ vertices to $X_0$, $X_1$, and $X_2$.  On the other hand, $\sum_{C\in C_{\star}} |C|\ge 3|C_{\star}|$ and, by \cref{degree_corners_and_leaves}, $\sum_{C\in C_{\star}} |\widehat{N}_H(C)|\ge 3|C_\star|$. \pat{Remark: this inequality is always loose, in fact $|\widehat{N}_H(C)|\ge 4|C|$}

  For each component $C$ in $C_{\boxtimes}$ we choose the vertex $x$ that dominates $C$, some vertex $w$ in $B(C)$ and some vertex $v\in B(H)$ adjacent to $w$.  We place $\{v,w,x\}$ in each of $X_0$, $X_1$, and $X_2$.  This contributes a total of $9|C_{\boxplus}|$ vertices to $X_0$, $X_1$, and $X_2$. On the other hand, $\sum_{C\in C_{\boxplus}} |C|\ge 5|C_{\star}|$ and, by \cref{degree_corners_and_leaves}, $\sum_{C\in C_{\star}} |\widehat{N}_H(C)|\ge 4|C_\star|$.

  For each component $C$ in $\overline{C}$, we apply \cref{two_connected_critical} to obtain sets $X_0',X_1',X_2',L_0',L_1',L_2'$.
  For each $j\in\{0,1,2\}$ we add $X_j'$ to $X_j$ and for each $w\in L_j'$ we choose a neighbour $w'$ of $w$ in $B_0$ and add it to $X_j$.  This ensures that $X_j$ dominates every vertex in $C$.  The total number of vertices added to $X_0$, $X_1$, and $X_2$ by this step is $\sum_{C\in\overline{C}}(|C|+|B(C)|+|V_1(C)|)$.  On the other hand, by \cref{degree_corners_and_leaves}, $\sum_{C\in \overline{C}} |\widehat{N}_H(C)|\ge \sum_{C\in \overline{C}} (|B(C)|+|V_1(C)|)$.

  Finally, it remains to consider the components of $H_1$ that are in $C_1$. Each of these components is a single vertex in $V_0(B_1)$. These we deal with as a group. For each $w\in V_0(B_1)$, let $H_w:=H[N_H[w]]$ and let $Q:=\bigcup_{w\in V_0(B_1)} H_w$.  For each $w\in V_0(V_1)$, the graph $H_w$ is isomorphic to $W_k$ for some $k\ge 3$, $B(H_w)\subseteq B(H)$ and, for any component $C$ other than the one containing $w$, $\widehat{N}_H(C)\cap B(H_w)=\emptyset$. In other words, none of the vertices in $B(Q)$ has been counted in the preceding paragraphs.  We construct a $X'\subseteq B(Q)\subseteq B(H)$ that dominates $I(Q)=V_0(V_1)$ by repeating the following as long as there is vertex $w$ in $V_0(B_1)$ not dominated by $X'$:
  \begin{compactenum}
    \item  If $H_w$ has no vertex in common with $H_{w'}$ for any $w'\in V_0(B_1)\setminus\{w\}$ then we add any vertex of $B(H_w)$ to $X'$.
