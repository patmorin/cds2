...
\begin{compactenum}[({DP}1)]
  ...
  \item $N_{H'}(w)\subseteq N_H(w)$ for all $w\in I(H)$. \label[dp]{same_neighbourhood}
\end{compactenum}
...
\begin{lem}\label{dom_minimal}
  For any generalized near-triangulation $H$, dom-minimal generalized near-triangulation $H'$ that dom-respects $H$.
\end{lem}
...
\pat{Much of this section can be simplified, since we don't care about degree-$1$ vertices in $H-B(H)$ anymore and we have lots of control over .}
