...
\begin{abstract}
  We show that every $n$-vertex triangulation has a connected dominating set of size at most $7n/15$.  Equivalently, every $n$ vertex triangulation has a spanning tree with at least $8n/15$ leaves. Prior to the current work, the best known bounds were $2n/3$ and $n/3$, respectively.  As an application of this, we show that every $n$-vertex planar graph has a one-bend non-crossing drawing in which some set of at least $8n/15$ vertices is drawn on the $x$-axis.
\end{abstract}

\vida{Note that if all the vertices of G are on a line then no edge in a 1-bend drawing crosses the line and thus G is subhamiltonian. Since not all planar graphs are subhamiltonian, that implies that the tight bound for the drawings above is strictly less than n. I wonder how much smaller than n it has to be. Is it a fraction of n? Do we know anything about the side of the largest induced subhamiltonian graph in a planar triangulation? Whatever we have on that line induces a subhamiltonian planar graph. So basically the result above also says that each planar graph has induced subgraph of size at least 8n/15 that is subhamiltonian planar. Do not know if that is interesting. n/4 follows trivially from 4-colour theorem. n/2 follows from the fact that every other layer of BFS induced an outerplanar graph.
...
\begin{thm}\label{one_bend_collinear}
  For every $n\ge 4$, every $n$-vertex planar graph has a $1$-bend collinear set of size at least $(8n+12)/15>0.53\overline{3}n$.
\end{thm}

\subsection{Related Work}

\pat{Add: \newline
results on edge-maximal outerplanar graphs \newline
...
\begin{proof}
  ...
  Since $H$ is a near-triangulation and $f$ is a face of $H[B(H)]$, $H$ has some face $v_j v_{j+1} z$ with $z$ in the interior of $f$.  By the definition of $j$, $z\neq w$.  Therefore, $N_H^+(v_j)\supseteq \{w,z\}$ and, since $\deg^+_H(v_j)\le 2$, $N_H^+(v_j))= \{w,z\}$.  Therefore $N^+_H(v_j)\subseteq N_H[w]$.  Since $f$ is a face of $H[B(H)]$, the only neighbours of $v_j$ in $B(H)$ are $v_{j-1}$ and $v_{j+1}$. Since $H$ is a near-triangulation, this implies that $w v_j z$ is a face of $H$.  In particular $wz$ is an edge of $H$.\pat{Add the edge $wz$ to the figure.}
...
\begin{proof}
  Each vertex $w$ in $B(H-B(H))$ is adjacent to at least one vertex $v_0$ on the outer face of $H$.  The vertex $v_0$ has two neighbours $v_{-1}$ and $v_1$ on the outer face of $H$. Since $H$ is dom-minimal, it contains faces $v_0v_1x$ and $v_0v_{-1}y$ with $x$ and $y$ being inner vertices of $H$.  Then at least one of $x$ or $y$ must be $w$ since, otherwise, $\deg^+_H(v_0)\ge 3$. (Note that the case $x=y\neq w$ is not possible since $v_0v_1x$ and $v_0v_{-1}y$ are each faces of $H$, which would leave no place for $w$.)   But then $w$ is adjacent to at least one of $v_{-1}$ or $v_1$.  Therefore $w$ is adjacent to at least two vertices in $B(H)$.  Therefore, each $w\in B(H-B(H))$ contributes at least $2$ to $\sum_{v\in B(H)}\deg^+_H(v)$, so $2|B(H)|\ge \sum_{v\in B(H)}\deg^+_H(v)\ge 2|B(H-H(B))|$, as required.
\end{proof}
...
\begin{proof}
  \pat{There is some work to do here.  We should break this up into cases where we eliminate eliminate bridges and cut vertices first.  Then, finally, we're left with the case where each component of $H-B(H)$ is a $2$-connected critical graph. To deal with this, we first deal with the stupid cases where this graph is just a $3$-cycle or a cycle with one inner vertex. (We can handle these by choosing any vertex on the face).  Otherwise the graph is an outerplanar graph with stars in some of its faces.  In each such face we'll contract one of the star edges (we should choose on incident to a chord of $H[B(H)]$, which is why we had to rule out the second stupid case) to we get an outerplanar graph and those special faces are star-triangulated.  When we contract, the vertex we contract into gets a weight of $3$. All other vertices get a weight of $2$. The total weight is then $2|B(H-B(H))|+|H-BB(H)|\le |H|/3$, by \cref{big_boundary}. Then we $3$-colour and take the lowest weight colour class.  The key point is that, for each weight-$2$ vertex $w$ we choose, we have to include $w$ and a neighbour $v$ of $w$ in $B(H)$.  For each weight $3$ vertex $w$ we choose, we have to take $w$, a neighbour $v$ of $w$ in $B(H)$, and the inner vertex of $H-B(H)$ that caused $w$ to have weight $3$.}
\end{proof}
...
\begin{proof}
  ...
  \end{equation}
  \pat{There was a not-so-little mistake in \cref{best_objective} that I fixed.  (I was forgetting to count the extra vertex that is added to $X$ each time Case~2 occurs.) Unfortunately now we get a bound of $10/21$, but still better than a half.}
