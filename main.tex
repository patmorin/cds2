...
\newtheorem{clm}{Claim}
\newenvironment{clmproof}{\noindent\emph{Proof of Claim:}}{\hfill$\triangle$\par}
\newtheorem{lem}{Lemma}
...
\begin{thm}\label{main_result2}
  For any $n\ge 4$, any $n$-vertex triangulation $G$ has a connected dominating set $X$ of size at most $7n/13 + O(1)$ and there exists an algorithm that finds $X$ in linear time.
\end{thm}
...
\begin{proof}
  ...
  \begin{equation}
      x_2 \le \sum_{t\ge 3}x_t + z + 1 \enspace .
  \end{equation}
  ...
  \]
  As in the previous proof, we combine these constraints with the constraints $n=|D_{r-1}|+|I_{r-1}|$ and $|B_{r-1}|\ge 3|I_{r-1}|$ to get the following collection of inequalities:
  \begin{align}
    \sum_{t\ge 2} x_t + z + 4I \le n \label{c_first}\\ 
    3 + \sum_{t\ge 2} tx_t + 2z + I \le n \label{c_second} \\ 
    3 + \sum_{t\ge 2}(t-1)x_t \ge 3I\label{c_third} \\ 
    x_2 \le \sum_{t\ge 3} x_t + z + 1 \label{c_fourth}
z  \end{align}
  and our goal is to maximize 
  \begin{align}
      \sum_{t\ge 2} x_t + z + I 
      & \le \sum_{t\ge 2} x_t + z + B/3 \notag \\
      & \le \sum_{t\ge 2} x_t + + z + \tfrac{1}{3}\left(3+ \sum_{t\ge 2}(t-1)x_t\right) \notag \\
      & = \tfrac{2}{3}\sum_{t\ge 2} x_t + z + 1 + \tfrac{1}{3}\sum_{t\ge 2}tx_t
      \label{objective2}
  \end{align} 
  The unbounded number of variables $x_t$, $t\ge 2$ is problematic. We resolve this with the following claim:
  \begin{clm}
    In any assignment of $I$, $B$, and $x_t$, $t\ge 2$ that maximizes \cref{objective2} subject to \cref{c_first,c_second,c_third,c_fourth}, $x_t=0$ for each $t\ge 3$.
  \end{clm}
  \begin{clmproof}
     If $x_t = c >0$ then we can set $x_t\gets 0$, $z\gets tc/2$, $B\gets B-2c$, and $I\gets I-2/t$.  This increases the value of \cref{objective2} and still satisfies \cref{c_first,c_second,c_third,c_fourth}.
  \end{clmproof}
  \pat{Continue here....Now everything gets easy...}
  \newline {\color{cyan} SO: we can prove $x_2 \ge z$. That is $x_2 \in \{z, z+1\}$.}
  ...
  % \]
  Now we just need to solve the actual LP by hand to get a rigorous proof.  This is a little bit annoying because we have infinitely many variables $x_t$, $t\ge 2$, but $x_t$ seems to be redundant for all $t\ge 3$. \pat{If someone is good at linear algebra, this would be really helpful.}
\end{proof}
