...
Note that if all the vertices of $G$ are on the $x$-axis then no edge in a $1$-bend drawing of $G$ crosses the $x$-axis and thus $G$ has a $2$-page book embedding. This implies that $G$ is a spanning subgraph of some Hamiltonian graph $G^+$.  The Goldner–Harary graph is an $11$-vertex triangulation that is not Hamiltonian, giving an example of a graph that has $1$-bend collinear set of size greater than $10n/11=0.90\overline{90}n$.jae
...
\begin{proof}
  ...
  \pat{Replace the following paragraph with charging argument?}
...
\end{thm}
\hussein{Is adding the following inequality to the constrains helpful  $r \leq |E|/4 \leq 3n/4 - 3/2$? I don t think so  }
\begin{proof}
