...
\begin{lem}\label{degree_isolated}
  Let $H$ be a generalized near-triangulation and let $C$ be a component of $H-B(H)$ that consists of a single vertex. Then $H$ contains at least $3$ edges with one endpoint in $C$ and one endpoint in $B(H)$.
\end{lem}

\begin{proof}
  Follows from the fact that $H$ is a generalized near-triangulation and the vertex in $C$ is an inner vertex of $H$.
\end{proof}
...
\begin{lem}\label{degree_walk}
  Let $H$ be a $2$-critical generalized near-triangulation, let $C$ be a component of $H-B(H)$ with at least two vertices and let $W$ be the boundary walk around the outer face of $H-B(H)$. Then $H$ contains at least $|W|$ edges with one endpoint in $C$ and one endpoint in $B(H)$.
\end{lem}

\begin{proof}
  Consider two consecutive edges $uv$ and $vw$ in some $W$, so the outer face of $H-B(H)$ is to the left of the path $uvw$.  Since $u,v,w$ are in $I(H)$, $H$ contains faces $uxv$ and $vyw$ that are to the left of $uv$ and $vw$, respectively. Since $uvw$ is in the boundary walk of the outer face of $C$ $C$, these faces are not faces of $H-B(H)$, so $x,y\in B(H)$.  We claim that $x\neq y$.  To see this, there are two cases to consider.  If $u\neq w$ and $x=y$ then $x\in B(H)$ is adjacent to each of $u,v,w\in I(H)$, so $\deg^+_H(x)\ge 3$, contradicting the fact that $H$ is $2$-critical  If $u=w$ and $x=y$, then there are two inner faces of $H$ that have $uv$ on their boundary, which is not possible because $H$ does not have parallel edges.  Therefore, the edges $vx$ and $vy$ can be assigned to the consecutive pair $uv$ and $vw$. We can do this to each of the consecutive pairs of edges without assigning any edge to more than one consecutive pair.
\end{proof}

For a generalized near-triangulation $H_1$, let $\mu(H_1)$ be two times the number of degree-$0$ vertices in $H_1$ plus the number of degree-$1$ vertices in $H_1$ plus the number of vertices in $B(H_1)$.
...
\begin{proof}[Proof Sketch]
  \pat{Watch out for biconnected components with two internal vertices.}
...
\begin{proof}
  Let $B_0:=B(H)$, let $H_1:=H-B(H)$, let $B_1:=B(H_1)$, let $I_1:=I(H_1)$,  let $L$ be the set of degree-$1$ vertices in $H_1$ and let $Z$ be the set of degree-$0$ vertices in $H_1$.
  ...
  For each biconnected component $C$ of $H-B(H)$ with at least $3$ vertices, we apply \cref{two_connected_critical} to obtain sets $X_0,X_1,X_2,L_0,L_1,L_2$.
  For each $w\in L_j$ then take a neighbour $w'$ of $w$ in $B_0$ and add it to $X_j$.  This ensures that $w$ is dominated by $X_j$.

  For each degree-$0$ vertex $w$ in $H-B(H)$ we will choose a vertex in $N_H(v)\cap B(H)$ and place $v$ in each of $X_0$, $X_1$, and $X_2$. This accounts for a total of $3|Z|$ vertices in $X_0$, $X_1$, and $X_2$.

  For each component $C$ of size $2$ in $H-B(H)$ we choose one vertex $v$ in $B(H)$ that dominates $C$ and use this to
