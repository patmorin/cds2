...
\begin{proof}
  By definition $N_H(w)\subseteq B(H)$ and, since $w$ is an inner vertex in a generalized near triangulation $|N_H(w)|\ge 3$.  Let $F$ be the face of $H[B(H)]$ that contains $w$ in its interior, so $V(F)=N_H(w)$.  We claim that $V(F)$ contains a cut vertex of $H$, that $V(F)$ contains a vertex $v$ with $N_H(v)=\{w\}$, or that $V(F)$ contains a pair of vertices $v$ and $v'$ with $N^+_H(v)=N^+_(v')$.  
  
  
  To prove this claim, let $F:=v_0,\ldots,v_{r-1}$.  If $\deg^+_H(v_i)=1$ for some $i\in\{0,\ldots,r\}$, then $N^+_H(v_i)=\{w\}$ and we are done.  Assume, therefore that $\deg^+_H(v_i)=2$ for each $i\in\{0,\ldots,r-1\}$. Thus, $v_0$ is adjacent to some vertex $x_0\in I(H)\setminus\{w\}$. Let $F_0$ be the face of $H[B(H)]$ that contains $x_0$ in its interior. Since $\deg_{H-B(H)}(w)=0$, $F\neq F_0$.  If $v_0$ is not a cut vertex of $H$, then the faces $F$ and $F_0$ have an edge in common.  Without loss of generality, assume that $F$ and $F_0$ have the edge $v_0v_1$ in common.  We claim that $F_0$
  
  .  Since $H[B(H)]$ is outerplanar  If $v_0$ is not a cut vertex of $H$, there is a cycle $w$ and $x_0$ are in the same component of $S$
  ...
  \end{figure}
  \begin{compactenum}[(a)]
    \item There exists $v\in N_H(w)$ with $\deg^+_H(v)=2$. In this case, $N^+_H(v)=\{w,w'\}$.  There are two cases to consider:
    \begin{compactenum}[(i)]
        ...
        \item The vertices $w$ and $w'$ are in the same biconnected component of $H$.  Let $F$ and $F'$ be the inner faces of $H[B(H)]$, where $F$ contains $w$ in its interior and $F'$ contains $w'$ in its interior. 
        
        Since $\deg_{H-B(H)}(w)=0$, $w$ is only vertex of $H$ in the interior of $F$.  The faces $F$ and $F'$ share an edge $vv'$.
        
        , one of which contains $w$ and the other of which contains $w'$.
    \end{compactenum}
