...
\begin{compactenum}
  \item We can directly complete the dominating set $X_r$ by adding a set $\Delta_{r-1}$ with at most $|G_i|/3$ additional vertices.
  \item We can add a pair of vertices $\{v_i,w_i\}$ that increase the size of the dominated set $N[X_{i+1}]$ by at least $5$ but only increase the size of the boundary set $B(G_{i+1})$ by at most $2$.
\end{compactenum}

\subsection{A Heuristic Analysis}
Before diving into the details, we present a heuristic analysis.  Roughly speaking, Case~1 above implies that each vertex left in $G_{r-1}$ contributes $1/3$ to the final dominating set $X_r$.  This implies that Case~2 above is more efficient than taking a vertex of inner-degree $3$.  Indeed, Case~1 has a cost $2+2/3=8/3$ but decreases the number of undominated vertices by $5$.  On the other hand, choosing a vertex of inner-degree $3$ has a cost of $1+3/3=2$ but only decreases the number of undominated vertices by $3$.  If we consider the progress over cost ratio, the former case has a ratio of $5/(8/3)=15/8=1.875$ while the latter case has a ratio of $3/2=1.5$. We formalize this by setting a linear program to bound the maximum size of the final dominating set $X_r$.

In the end, the theoretical worst-case for the resulting algorithm is that it chooses inner-degree $3$ vertices for the first $r-1$ consecutive rounds, resulting in a graph $G_{r-1}$ with $|B(G_{r-1})|\approx 2r$ and $|I(G_{r-1})|\approx n-3r$, so $|G_{r-1}|\approx 2r+n-3r=n-r$.  The algorithm then finishes in Case~1, above, by taking a set $\Delta_{r-1}$ of size $(n-r)/3$.  This results in a dominating set $X_r$ of size roughly $r+(n-r)/3=n/3+(2/3)r$. This is a strictly increasing function of $r$, but the value of $r$ is constrained by the fact that $G_{r-1}$ has no vertices of inner-degree $3$, which implies that $|B(G_{r-1})|\ge |I(G_{r-1})|$.  This translates to the constraint $2r \le n-3r$ or $r \le n/5$.  Thus, the final dominating set has size at most $n/3 + (2/3)n/5=7n/15$.  We now make this heuristic analysis more formal.
