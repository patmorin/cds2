...
\subsection{Dom-Minimal and Dom-Preserving subminors}
...
\begin{compactenum}[({DM}1)]
    ...
    \item if $v\in B(H)$ has $\deg^+_H(v)=1$, then $H[N_H[v]]$ is isomorphic to $K_4$. \label[dm]{inner_degree_1}
...
\pat{Define minor and subminor}
A subminor $H'$ of a generalized near-triangulation $H$ is \defin{dom-preserving} if
\begin{compactenum}[({DP}1)]
  \item $H'$ is a generalized near-triangulation; \label[dp]{same_class}
  ...
  \item $N_{H'}(v)=N_H(v)\cap V(H')$ for all $v\in I(H)$. \label[dp]{same_neighbourhood}
\end{compactenum}

% \begin{obs}
%   Let $H$ be a generalized near-triangulation, let $H'$ be a dom-preserving subminor of $H$, and let $\Delta$ be a subset of $V(H)$ that dominates $I(H)$.  Then $\Delta\cap V(H')$ dominates $I(H)$.
% \end{obs}
%
% \begin{proof}
...
\begin{lem}\label{dom_minimal}
  For any generalized near-triangulation $H$, there exists a dom-preserving subminor $H'$ of $H$ that is dom-minimal.
\end{lem}

\begin{proof}
  The proof is by induction on $|V(H)|+|E(H)|$.  If $H$ is already dom-minimal, then setting $H'=H$ satisfies the requirements of the lemma, so assume that $H$ is not dom-minimal.  It is straightforward to verify that the dom-preserving subminor relationship is transitive, so if $H$ has a dom-preserving subminor $H^*$ and $H^*$ has a dom-preserving subminor $H'$ then $H'$ is a dom-preserving subminor of $H$.  Therefore, it is sufficient to show the existence of a dom-preserving subminor $H^*$ of $H$ with fewer edges or fewer vertices than $H$, and the inductive hypothesis provides the desired dom-minimal dom-preserving subminor $H'$ of $H$.

  If $H$ contains a vertex $v\in B(H)$ with $\deg^+_H(v)=0$ then $H-v$ is a dom-preserving subminor of $H$ with fewer vertices than $H$.  We now assume that $\deg^+_H(v)\ge 1$ for all $v\in B(H)$.  Since $H$ is not dom-minimal then $H$ contains a biconnected component $C$ that is not dom-minimal.
  \begin{compactenum}
    \item If there exists an edge $vw$ on the outer face of $C$ that is not incident to any inner face $vwx$ with $x\in I(C)$ then $B(H-vw)=B(H)$ and $I(H-vw)=I(H)$, and $H-vw$ is a is dom-preserving subminor of $H$ that has fewer edges than $H$.

    \item If there exists a vertex $v\in B(C)$ with $\deg^+_C(v)=0$ then $v$ is incident to an edge $vw$ that is on the outer face of $C$ and on the outer face of $H$. Since $\deg^+_C(v)=0$, $vw$ is not incident to any inner face $vwx$ with $x\in I(C)$ and we can proceed as in the previous case.

    \item If there exists a vertex $v\in B(C)$ with $\deg^+C(v)=1$ then $H$ contains faces $xvw$ and $vyw$ where $x$ and $y$ are on the outer face of $C$ and $w$ is an inner vertex. If $H[N_H[v]]$ is not isomorphic to $K_4$, then $xy\not\in E(H)$.  In this case, we contract $v$ into $x$ to obtain dom-preserving subminor of $H$ with fewer vertices than $H$. \qedhere
  \end{compactenum}
\end{proof}
...
\begin{lem}\label{leaf_killer}
  Let $H$ be a dom-minimal generalized near-triangulation such that $\deg^+_H(v)\le 2$ for all $v\in B(H)$ and such that $H-B(H)$ contains a vertex $w$ with $\deg_{H-B(H)}(w)=1$.  Then there exists $v\in B(H)$ and a dom-preserving subminor $H'$ of $H-v$ such that $|H'|\le |H|-3$ and $|B(H')|\le |B(H)|-1$.
\end{lem}
...
\begin{lem}\label{degree_zero_killer}
  ...
  \begin{compactenum}[(a)]
    \item there exists $v\in B(H)$ and a dom-preserving subminor $H'$ of $H-v$ such that $|H'|\le |H|-3$ and $|B(H')|\le |B(H)|-1$; or \label[p]{kill_pair}
    \item there exists $v\in B(H)$ and a dom-preserving subminor $H'$ of $H-v$ such that $|H'|\le |H|-4$ and $|B(H')|\le |B(H)|-3$. \label[p]{kill_k4}
  \end{compactenum}
\end{lem}
...
\begin{proof}
  ...
  \begin{compactenum}[(a)]
    \item There exists $v\in N_H(w)$ with $\deg^+_H(v)=2$. In this case, $N^+_H(v)=\{w,w'\}$ and the vertices $w$ and $w'$ are in a different biconnected components $C$ and $C'$, respectively, of $H$. The component $C$ has exactly one inner vertex, $w$ so, by \cref{inner_degree_1}, $C$ is isomorphic to $K_4$.  By \cref{inner_degree_1}, $C'[N_{C'}(v)]$ is also isomorphic to $K_4$.  We claim that $C'$ is isomorphic to $K_4$ indeed, the only other possibility is that the triangle $xw'v$ contains inner vertices.  If $xw'v$ contains more than one inner vertex then  $\deg^+_H(x)>2$ or $\deg^+_H(y)>2$, which is ruled out by the statement of the lemma.  If $xw'v$ contains exactly one inner vertex then $\deg_{H-B(H)}(w')=1$, which is ruled out by the statement of the lemma.

    Let $H'$ be a dom-preserving subminor of $H-v$ that is dom-minimal. Then $w$ and $w'$ are both in $B(H-v)$.   Since $\deg^+_{H-v}(w)=\deg^+_{H-v}(w')=0$, neither $w$ nor $w'$ are in $H'$. Therefore $|H'|\le |H-\{v,w,w'\}|=|H|-3$.  Also $B(H-v)=B(H)\setminus\{v\}\cup\{w,w'\}$ so, by \cref{boundary_subset}, $|B(H')|\le |B(H)|-1$.

    \item $\deg^+_H(v)=1$ for all $v\in N_H(w)$.  In this case let $v$ be any vertex in $N_H(w)$ and let $H'$ be a dom-preserving subminor of $H-v$ that is dom-minimal.  Then $w\in B(H-v)$, so $\deg^+_{H-v}(v')=0$ for all $v'\in N_H(w)$. Since $H'$ is dom-minimal, $N_H(w)\cap V(H')=\emptyset$, by \cref{bad_vertex}.  Similarly $w\not\in V(H')$, also by \cref{bad_vertex}. Therefore $|H'|\le |H-(\{w\}\cup N_H(w))| \le |H|-4$. Then $B(H-v)=B(H)\setminus\{v\}\cup\{w\}$.
