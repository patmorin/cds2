...
\author{%
  ...
  Vida Dujmović\thanks{School of Computer Science and Electrical Engineering, University of Ottawa. Research partially funded by NSERC and a University of Ottawa Research Chair}\and
...
\begin{abstract}
  We show that every $n$-vertex triangulation has a connected dominating set of size at most $10n/21$.  Equivalently, every $n$ vertex triangulation has a spanning tree with at least $11n/21$ leaves.   Prior to the current work, the best known bounds were $n/2$, which follow from the work of Albertson, Berman, Hutchinson, and Thomassen (J. Graph Theory \textbf{14}(2):247--258, 1990). One immediate consequence of this result is an improved bound for the SEFE without mapping graph drawing problem studied by Angelini, Evans, Frati, and Gudmundsson (J. Graph Theory \textbf{82}(1):45--64, 2016).  As a second application, we show that for every set $P$ of $\lceil 11n/21\rceil$ points in $\R^2$ every $n$-vertex planar graph has a one-bend non-crossing drawing in which some set of $11n/21$ vertices is drawn on the points of $P$.  The main result extends to $n$-vertex genus-$g$ surface triangulations without parallel edges or self-loops and implies that these have connected dominating sets of size at most $10n/21+O(\sqrt{gn})$.
\end{abstract}
\end{titlepage}

\section{Introduction}

A set $X$ of vertices in a graph $G$ is a \defin{dominating set} of $G$ if each vertex of $G$ is in $X$ or adjacent to a vertex in $X$.\footnote{Any graph $G$ that we consider in this paper is a finite undirected graph without self-loops and without parallel edges, with vertex set $V(G)$ and edge set $E(G)$.}  A dominating set $X$ of $G$ is \defin{connected} if the subgraph $G[X]$ of $G$ induced by the vertices in $X$ is connected.  There is an enormous body of literature on dominating sets. Several books are devoted to the topic \cite{haynes.hedetniemi.ea:domination,haynes.hedetniemi.ea:topics,du.wan:connected,haynes.hedetniemi.ea:vol2}, including a book and book chapter devoted to connected dominating sets \cite{du.wan:connected,chellali.favaron:connected}.
% \vida{should we note that only connected graphs have connected dominating sets, thus we study he problem in connected and simple graphs}
% \pat{I vote no.}
...
% \vida{Added this paragraph. Wanted to somehow immediatly make a connection. Do we keep it or?}
Connected dominating sets are complementary to the leaves of spanning trees: a set of vertices $X$ is a connected dominating set of a graph $G$ if and only if $G$ has a spanning tree in which the vertices of $G-X$ are all leaves. Thus, connected dominating sets are studied implicitly in the literature on spanning trees with many leaves \citep{DBLP:journals/jgt/DingJS01, kleitman.west:spanning, kleitman.west:spanning, doi:10.1137/100801251,doi:10.1137/060664318, DBLP:conf/esa/BousquetI0MOSW20, noguchi.zamfirescu:spanning}. A closely related and extensively studied notion is that of spanning trees with no degree two vertices, known as \emph{homeomorphically irreducible spanning trees} \citep{chen.ren.ea:homeomorphically,albertson.berman.ea:graphs,CHEN2013409, DBLP:journals/jgt/ItoT22, DBLP:journals/jgt/Hoffmann-Ostenhof18a, gessel2023good}.  Since an $n$-vertex tree with no degree $2$-vertices has at least $n/2+1$ leaves, the existence of a homeomorphically irreducible spanning tree implies the existence of a spanning tree with at least $n/2+1$ leaves (and the existence of a connected dominating set of size at most $n/2-1$).
% % as
%  they imply $n/2$ upper bound on connected dominating sets and $n/2$ lower bound on the number of leaves, as discussed in the next section.

% \vida{Also, our problem is related to Good Will Hunting problem :D \citet{gessel2023good}}

The focus of the current paper is on the existence of small connected dominating sets in edge-maximal planar graphs, a.k.a triangulations, and their applications.  In the following subsections we describe the history of this problem and two applications.



% \pat{Insert sentence here introducing the next section.}
...
% $T$ with no vertices of degree $2$ (that is, homeomorphically irreducible spanning trees), which implies that the number of leaves of $T$ exceeds the number of non-leaves by at least $2$.  In particular, the number of leaves is greater than $(n+2)/2$. Thus, the set $X$ of non-leaf vertices of $T$ is a connected dominating set of size at most $(n-2)/2<0.5n$.
\citet{chen.ren.ea:homeomorphically} give a significant generalization of this result, which applies to any connected graph $G$ in which the graph induced by the neighbours of each vertex is connected.
Answering an open problem by \citet{albertson.berman.ea:graphs} and confirming the conjecture of Archdeacon \citep{Gross_Tucker_2009}, \citet{CHEN2013409} generalize this result to graphs in which every edge is in at least two $3$-cycles.


Motivated by the fact that this $0.5n$ bound has stood for over 30 years, several authors have asked if it can be improved.  \citet[Question~4.2]{bradshaw.masarik.ea:robust} ask if it can be improved to $n/3$.
 \citet{noguchi.zamfirescu:spanning} ask if it can even be improved to $(\tfrac{1}{2}-\epsilon)n$ for any $\epsilon>0$, even in the special case of $4$-connected triangulations. We prove:
...
\begin{thm}\label{main_result2}
  For every $n\ge 3$, every $n$-vertex triangulation $G$ has a connected dominating set $X$ of size at most $10n/21= 0.\overline{476190}n$.  Equivalently, $G$ has a spanning tree $T$ with at least $11n/21=0.\overline{523809}n$ leaves.  Furthermore, there exists an $O(n)$ time algorithm for finding $X$ and $T$.
\end{thm}
...
For bounded-degree planar graphs, this result can be improved to $|Y|=\Omega(n^{0.8})$ \cite{dujmovic.morin:dual}.  Determining the supremum value of $\alpha$ such that every $n$-vertex planar graph has a collinear set of size $\Omega(n^{\alpha})$ remains a difficult open problem, but it is known that $\alpha \le \log_{23}(22)<0.9859$ \cite{ravsky.verbitsky:collinear}. A history of free sets and their applications in graph drawing and related areas is surveyed by \citet{dujmovic.morin:free}.

It is common in graph drawing to consider non-crossing drawings of a graph $G$ in which each edge of $G$ is represented by a polygonal chain consisting of at most $k+1$ line segments.  Such a representation is called a \defin{$k$-bend drawing} of $G$ (so a straight-line drawing is a $0$-bend drawing). A subset $Y$ of $V(G)$ is a \defin{$k$-bend free set} if, for every $|Y|$-point set $P$,  $G$ has a $k$-bend drawing in which the vertices of $Y$ are mapped to the points in $P$. A result of \citet{DBLP:journals/combinatorica/FraysseixPP90} shows that, for any planar graph $G$, $V(G)$ is a $2$-bend free set, so every $n$-vertex planar graph has a $2$-bend free set of size $n$. This leaves open the question of $1$-bend free sets.  We show that, for any spanning tree $T$ of $G$, the leaves of $T$ are a one-bend free set of $G$.  Combined with \cref{main_result2}, this gives:
...
%\vida{INCLUDING ABSTRACT. Aha. I see that you killed it in abstract already}

% \pat{Remove this paragraph.}\vida{or rather move to the section about Frati application}
% In order to resolve a graph drawing problem (discussed further in \cref{related_work}), \citet{angelini.evans.ea:sefe} gave another proof of the $0.5n$ bound in triangulations by showing that every plane graph contains an induced outerplane graph of size at least $0.5n$.

A planar graph equipped with a non-crossing embedding is called a \defin{plane graph}.  We treat any subgraph $G'$ of a plane graph $G$ as a plane graph that inherits its embedding from $G$. A plane graph in which all vertices appear on a single face is called an \defin{outerplane} graph.  Motivated by a graph drawing problem (SEFE without mapping), \citet{angelini.evans.ea:sefe} prove the following result:

\begin{thm}\label{induced_outerplane_orig}
...
Observe that if $G$ is a connected plane graph and $X$ is a connected dominating set of $G$, then the vertices of $X$ are all contained in the interior of a single face $F$ of $G-X$.  Since $X$ is a dominating set, every vertex of $G-X$ is on the face $F$.  Thus, all vertices of $G-X$ are on a single face, so $G-X$ is an induced outerplane graph.\footnote{In \cref{discussion}, we show that the converse of this statement is almost true: the largest $Y\subset V(G)$ such that $G[Y]$ is outerplane and the smallest connected dominating set $X\subset V(G)$ satisfy $|X|+|Y|=|V(G)|$.}  Combined with \cref{main_result2}, this implies the following improvement of \cref{induced_outerplane_orig}:

\begin{cor}\label{induced_outerplane_improved}
    Every connected $n$-vertex plane graph $G$ contains an induced outerplane graph $G'$ with at least $11n/21=0.\overline{523809}n$ vertices, and there exists an $O(n)$ time algorithm to find the graph $G'$.
\end{cor}

\Cref{induced_outerplane_improved} immediately implies an improved result for the graph drawing problem considered by \citet{angelini.evans.ea:sefe}, improving the bound from $n/2$ to $11n/21$:
...
% If $G$ is a triangulation with $n\ge 4$ vertices and $G[L]$ is outerplane, then $G$ contains a set $L'$ of size at least $L$ such that $G[L']$ is an outerplane graph whose inner faces are all triangles.  Then $X:=V(G)\setminus L'$ is a connected dominating set of $G$.
...
% \pat{I don't care about anything from this section. I'm happy to cut it entirely.}\vida{I agree. We can cut this. }
...
% The $x$-axis is just one example of an $x$-monotone function (the function $f(x)=0$). Unsurprisingly, perhaps, this function turns out to be the most difficult for one-bend graph drawing.
...
In \cref{full_result} we show that a more careful construction leads to a proof of \cref{main_result2}.  In \cref{bounded_genus} we prove \cref{genus_result}.  In \cref{one_bend}, we discuss the connection between connected dominating sets and one-bend collinear drawings that leads to \cref{one_bend_collinear_thm}.  Finally, \cref{discussion} concludes by pointing out directions for future work.
...
\end{compactenum}
We say that a generalized near-triangulation $H$ is \defin{dom-minimal} if each of its biconnected components is dom-minimal.
...
\begin{proof}
  ...
  \begin{compactenum}
    ...
    % \hussein{do we mean $B(H^*) \subseteq B(H)$.} \pat{I think $B(H^\star)=B(H)$, no?} \hussein{Here if $v$ is not a vertex cut like the right most picture, then $v$ is deleted for ever. Right?}
    % \pat{As written above, $v$ is a vertex of $H^*$ on the outer face of $H^*$, so $B(H^*)=B(H)$.  Of course, $H^*$ is not dom-minimal (because $\deg^+_{H^*}(v)=0$), but that's ok it only has to dom-respect $H$ and have fewer edges.}
    % \hussein{Ok, I thought we are removing the isolated vertex too.}
  \end{compactenum}
\end{proof}
...
\begin{figure}
  ...
  \caption{Removing an inner-degree $2$ vertex $v$ is immediately followed by removing an inner-degree $3$ vertex $w$ and the vertex $u$ is not in a dom-minimal graph that dom-respects $H-\{v,w\}$.}
  \label{2_3_figure}
\end{figure}
...
\begin{lem}\label{degree_zero_killer}
  Let $H$ be a dom-minimal generalized near-triangulation such that $\deg^+_H(v)\le 2$ for all $v\in B(H)$, $H-B(H)$ is critical, and $H-B(H)$ contains a vertex $w$ with $\deg_{H-B(H)}(w)=0$ but does not contain any vertex $w'$ with $\deg_{H-B(H)}(w')=1$.  Then
    ...
    % ; or \label[p]{kill_pair}
    % \item there exists $v\in B(H)$ and a graph $H'$ that dom-respects $H-v$ such that $|H'|\le |H|-4$ and $|B(H')|\le |B(H)|-3$. \label[p]{kill_k4}
    % % \item \pat{Unnecessary} there exists $u,v\in B(H)$ and a graph $H'$ that dom-respects $H-\{u,v\}$ such that $|H'|\le |H|-6$ and $|B(H')|\le |B(H)|-1$. \label[p]{kill_double}
\end{lem}
...
\begin{proof}
  We may assume that $H$ is connected, otherwise we can apply the lemma to one of the components of $H$ that contains a vertex $w\in I(H)$ with $\deg_{H-B(H)}(w)=0$. Let $F_w$ denote the face in $H[B(H)]$ that contains $w$ in its interior.  Since $H$ is a generalized near triangulation and $N_H(w)\subseteq B(H)$, it follows that $V(F_w)=N_H(w)$.
   % Observe that there exists some $w\in W$ and some edge $uv\in E(F_w)$ such that all the vertices in $W$ are contained in a single component of $H-\{u,v\}$. This choice of $w$ and $uv$ will be important in Case~(iii), below.
  % \pat{Does this need more justification?} \hussein{I understand it, but i did not see when we used it, or why it is important.} \pat{It's used in Case~(iii) to argue that $\deg^+_H(x)=2$}
  % \pat{You were right, Hussein, we didn't need it, which is good because it make the linear time algorithm more complicated.}
  ...
  \begin{figure}%[htpb]
    ...
    \begin{tabular}{ccc}
      ...
      \includegraphics[page=1]{figs/degree_zero_killer} &
      \includegraphics[page=4]{figs/degree_zero_killer} &
      \includegraphics[page=7]{figs/degree_zero_killer} \\
      % $H$ & $H$ & $H$ & $H$ \\
      $\Downarrow$ & $\Downarrow$ & $\Downarrow$  \\
      % \includegraphics[page=2]{figs/degree_zero_killer} &
      \includegraphics[page=2]{figs/degree_zero_killer} &
      \includegraphics[page=5]{figs/degree_zero_killer} &
      \includegraphics[page=8]{figs/degree_zero_killer} \\
      % $H-x$ & $H-x$ & $H-v$ & $H-v$ \\
      $\Downarrow$ & $\Downarrow$ & $\Downarrow$  \\
      \includegraphics[page=3]{figs/degree_zero_killer} &
      \includegraphics[page=6]{figs/degree_zero_killer} &
      \includegraphics[page=9]{figs/degree_zero_killer}
      % $H'$ & $H'$ & $H'$ & $H'$
    \end{tabular}
    % \begin{tabular}{lccc}
    %   (i)&\raisebox{-0.5\height}{\includegraphics[page=1]{figs/degree_zero_killer}} &
    %   \raisebox{-0.5\height}{\includegraphics[page=2]{figs/degree_zero_killer}} & \raisebox{-0.5\height}{\includegraphics[page=3]{figs/degree_zero_killer}} \\
    %   (ii)&\raisebox{-0.5\height}{\includegraphics[page=4]{figs/degree_zero_killer}} &
    %   \raisebox{-0.5\height}{\includegraphics[page=5]{figs/degree_zero_killer}} &
    %   \raisebox{-0.5\height}{\includegraphics[page=6]{figs/degree_zero_killer}} \\
    %   (iii)(a)&\raisebox{-0.5\height}{\includegraphics[page=7]{figs/degree_zero_killer}} &
    %   \raisebox{-0.5\height}{\includegraphics[page=8]{figs/degree_zero_killer}} &
    %   \raisebox{-0.5\height}{\includegraphics[page=9]{figs/degree_zero_killer}} \\
    %   (iii)(b)&\raisebox{-0.5\height}{\includegraphics[page=10]{figs/degree_zero_killer}}  &
    %   \raisebox{-0.5\height}{\includegraphics[page=11]{figs/degree_zero_killer}}  &
    %   \raisebox{-0.5\height}{\includegraphics[page=12]{figs/degree_zero_killer}} \\
    %   (iii)(c)&\raisebox{-0.5\height}{\includegraphics[page=13]{figs/degree_zero_killer}}  &
    %   \raisebox{-0.5\height}{\includegraphics[page=14]{figs/degree_zero_killer}}  &
    %   \raisebox{-0.5\height}{\includegraphics[page=15]{figs/degree_zero_killer}} \\
    %   & $H$ & $H-v$ & $H'$
    % \end{tabular}
  ...
  \begin{enumerate}[(i)]
    ...
    % \begin{compactenum}[(a)]
      ...
      % \item    If $x'\in B(H)$ then $N^+_H(x)=\{w'\}$ and we consider a dom-minimal graph $H'$ that dom-respects $H-v$.  \pat{No! $x$ may belong to a second bicomponent, making $\deg^+H(x)=2$.} Then $V(H')\subseteq V(H)\setminus\{w,v,x\}$, so $|H'|\le |H|-3$ and $B(H')\subseteq B(H)\setminus\{v,x\}\cup\{w'\}$ so $|B(H')|\le |B(H)|-1$, which satisfies the requirements of the lemma.

    %
    %
    %
    %   Therefore $N^+_H(x)\supseteq\{w',x'\}$ and, since $\deg^+_{H}(x)\le 2$, there exists an inner face $xx'x''$ with $x''\in B(H)$.  In this case, let $H_0$ be the graph obtained from $H-v$ by removing $x$ and adding the edge $x''w'$.  Note that $H_0$ dom-respects $H-v$.  Now let $H'$ be a dom-minimal graph that dom-respects $H_0$ (and therefore also dom-respects $H-v$). Then $V(H')\subseteq V(H)\setminus\{w,v,x\}$, so $|H'|\le |H|-3$ and $B(H')\subseteq B(H)\setminus\{v,x\}\cup\{w'\}$ so $|B(H')|\le |B(H)|-1$, which satisfies the requirements of the lemma. \qedhere
    % \end{compactenum}
    %
    %
    % $N_{H-B(H)}(w')$ contains a vertex $w''$ adjacent to $x$ which forms an inner face $xw''x'$ with $x'\in B(H)$. Note that $x'\neq y$, since this would imply that $\deg_{H-B(H)}(w')=\deg_{H-B(H)}(w'')=1$, which is ruled out by the statement of the lemma.  In this case, consider the graph $H_0$ obtained from $H-v$ by removing $x$ and adding the edge $x'w'$.  Then $H_0$ dom-respects $H-v$ and we let $H'$ be a dom-minimal subgraph that dom-respects $H_0$.
    %
    ...
    % We claim that $N^+_H(v)$ and $N^+_H(z)$ are disjoint.
    % By assumption $N^+_H(u)\cap N^+H(v)=\{w\}$, so $w'\not\in N^+_H(u)$.  \pat{Finish this thought.}

    % Let $w''$ be the unique vertex in $N^+_H(x)\setminus\{w'\}$.  Observe that $w''\neq w$ since $w$ is contained in the interior of $F_w$ and $x\not\in V(F_w)$. Let $H'$ be a dom-minimal graph that dom-respects $H-v$.  Notice that $w''$ is an inner vertex of $H-v$, so $w''$ is an inner vertex of $H'$, by \cref{same_inner_vertices}.
    %
    % We will now prove that $x\not\in V(H')$. See \cref{no_x}  Suppose, for the sake of contradiction, that $x\in V(H')$.  Then $\deg^+_{H'}(x)=1$ so, by \cref{inner_degree_1} $H'[N_H'[x]]$ is isomorphic to $K_4$ and $w''$ is the inner vertex of this $K_4$.  Let $x'$ be a vertex of $N_{H'}(x)\setminus\{w',w''\}$.  Then $x'$ is on the outer face of $H'$ so, by \cref{same_inner_vertices}, $x'$ is on the outer face of $H$.  Let $y'$ be the unique vertex in $N_{H'}(x)\setminus\{x',w''\}$. (Possibly, $y'=w'$, which occurs if and only if $w'\in V(H')$.)
    %
    % \begin{figure}
    %   \centering
    %   \includegraphics{figs/no-x}
    %   \caption{Proof that $x\not\in V(H')$}
    %   \label{no_x}
    % \end{figure}
    %   % Note that $y'$ may be equal to $w'$.
    %
    % By our choice of $w$ and $uv$, $|N_{H-B(H)}(w'')|\ge 1$.  The statement of the lemma assumes that $|N_{H-B(H)}(w'')|\neq 1$, so $|N_{H-B(H)}(w'')|\ge 2$.  By \cref{same_inner_vertices} $I(H')=I(H-v)=I(H)\setminus\{v,w,w'\}$. Therefore, $N_{H'-B(H')}(w'')=N_{H-B(H)}(w'')\setminus \{w'\}$, so $|N_{H'-B(H')}(w'')|\ge 1$.  Therefore, one of the inner faces of $H'[N_H'[x]]$ contains a neighbour $q$ of $w''$.  Since $N^+_H(x)=\{w',w''\}$, \cref{useful_little_guy} implies that $q$ is contained in the interior of $x'y'w''$. Then \cref{useful_little_guy} implies that $N_{H'}(x')$ contains a vertex $p$ in the interior of $x'y'w''$. (Possibly $p=q$).  If $y'=w'$, then $N^+_H(x')=N^+_{H'}(x')\supseteq\{w',w'',p\}$, which contradicts the assumption that $\deg^+H(x)\le 2$.  If $y'\neq w'$ then $y'$ is on the outer face of $H$.  Since, $N_{H-B(H)}(q)$ contains $w''$ and $|N_{H-B(H)}(q)|\neq 1$, some neighbour $q'$ of $q$ is also in the interior of $x'y'w''$.  Then \cref{useful_little_guy} implies that at least one of $x'$ or $y'$ has at least two neighbour in the interior $x'y'w''$.  Since each of $x'$ and $y'$ is adjacent to $w''$, this implies that $\deg^+H(x')\ge 3$ or $\deg^+_H(y')\ge 3$, a contradiction.  Therefore $x\not\in V(H')$.
    %
    % Since $x\not\in V(H')$, $V(H')\subseteq V(H)\setminus\{v,w,x\}$, and $B(H')\subseteq B(H)\cup N_H(v)\setminus\{v,w,x\}$ so $|V(H')|\le |V(H)|-3$ and $|B(H')|\le|B(H)|+2-3=|B(H)|-1$.\qedhere
    %
    %
    %
    %
...
% \hussein{Here, I think this isomorphism is caused by being critical not necessarily dom-minimal.} \pat{No, this graph is $2$-critical, but has a vertex of inner degree 1.\\ \includegraphics{figs/2-critical-not-degree-2}}
...
\begin{compactenum}[{[}bg1{]}]
    ...
    \item Otherwise, if $G_i-B(G_i)$ contains a vertex of degree $1$ we set $\Delta_i:=\{v_i\}$ where $v_i$ is the vertex $v$ guaranteed by \cref{leaf_killer}.\label[bg]{bg_leaf}
    \item Otherwise, if $G_i-B(G_i)$ contains a vertex of degree $0$ we set $\Delta_i:=\{v_i\}$ where $v_i$ is the vertex $v$ guaranteed by \cref{degree_zero_killer}.\label[bg]{bg_degree_zero}
...
\Cref{better_greedy} establishes the combinatorial result in \cref{main_result2} and the following theorem establishes the algorithmic result.
...
% \hussein{Our definition of 2-critical, is not when bg1,2,3,4 do not apply. Rather than all the element in $\zeta$ either in bucket $\zeta_1$ or $\zeta_2$.}
% \pat{If \cref{bg_high_degree} doesn't apply, then $\deg^+_{G_i}(v)\le 2$ for all $v\in B(G_i)$---that's (2-C1).  If \cref{bg_two_three} doesn't apply then $G-B(G_i)$ is critical---that's (2-C2).  If \cref{bg_leaf,bg_degree_zero} don't apply then $\deg_{G_i-B(G_i)}(w)\ge 2$ for all $w\in V(G_i-B(G_i))$---that's (2-C3).}

% \hussein{The thing is bg4 applies in the critical cases in our figures. This why I wanted to define critical differently.}



% \pat{There is an issue, though, and it's that I didn't specify what the counter values are in $\zeta$, and I was using them two different ways:  To deal with \cref{bg_leaf,bg_degree_zero} we need to know $\deg_{G_i-B(G_i)}(w)=\delta_i(w)$.  But to deal with \cref{bg_two_three} we need to know $\deg^+_{G_i-B(G_i)}(w)$.
%  This means we need a third sorted counter structure $\Phi$ that stores $\deg^+_{G_i-B(G_i)}(w)$ for the vertices in $B(G_i-B(G_i))$.  I've added this.}
