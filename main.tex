...
\begin{proof}
  \pat{There is some work to do here.  We need to do a star triangulation on each face of $H_0[B(H_0)]$ and then argue that, if we choose the colour class on the high-degree vertex then we can afford to also take the interior vertex.  Faces of size $4$ are really tight, but we will still be able to get $1/3$.}
  Let $H_0:=H-B(H)$ and let $H_1$ be a graph obtained by triangulating the inner faces of $H_0[B(H_0)]$. Since $H_1$ is outerplane, it has a proper $3$-colouring.  Let $X$ be the smallest colour class in this colouring.  Each face of $H_1$ contains at least one vertex in $X$, so 
  
  Now $3$-colour $H_0$ and let $X$ be the smallest colour class.
