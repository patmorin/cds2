...
% \date{}
\author{Anonymous}{No Affiliation}{email@anon.org}{orcidid}{No funding}
...
\Copyright{The authors} %TODO mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/

\ccsdesc[100]{\textcolor{red}{Replace ccsdesc macro with valid one}} %TODO mandatory: Please choose ACM 2012 classifications from https://dl.acm.org/ccs/ccs_flat.cfm

\keywords{connected domination, triangulations, planar graphs, graph drawing, collinear sets} %TODO mandatory; please add comma-separated list of keywords
...
\begin{cor}\label{main_theorem_cor}
  For every $n\ge 4$, every $n$-vertex triangulation $G$ has a spanning tree $T$ with at least $11n/21= 0.\overline{523809}n$ leaves.
\end{cor}
...
We consider a relaxation of this problem in which the edges of the graph can be drawn as a polygonal path consisting of at most two line segments.  Such a drawing is called a \defin{one-bend} drawing of $G$.  A subset $Y$ of $V(G)$ is a \defin{one-bend collinear set} if $G$ has a one-bend drawing in which all vertices of $Y$ appear on the $x$-axis.  We show that, for any connected dominating set $X$ in a triangulation $G$, the set $Y:=V(G)\setminus X$ is a one-bend collinear set of $G$.  Combined with \cref{main_result2}, this gives:

\begin{thm}\label{one_bend_collinear_thm}
  For every $n\ge 4$, every $n$-vertex planar graph has a one-bend collinear set of size at least $11n/21=0.\overline{523809}n$.
\end{thm}
...
\textbf{Connected Dominating Sets:}
...
\citet{DBLP:journals/comgeo/GiacomoDLW05} show that for \defin{any} strictly concave function $f:[0,1]\to\R$ and any planar graph $G$ there exists a function $x:V(G)\to[0,1]$ such that $G$
...
\begin{obs}\label{useful_little_guy}
  ...
  \begin{compactenum}
    \item If the interior of $xyz$ contains at least one vertex of $H$, then each of $x$, $y$, and $z$ has at least one neighbour in $H$.

    \item If the interior of $xyz$ contains at least two vertices of $H$, then at least two of $x$, $y$, and $z$ have at least two neighbours in $H$.
  \end{compactenum}
\end{obs}
...
% More precisely, by setting $x:=r$ and $y:=|I_{r-1}|$, the maximum size of $X_r$ is upper-bounded by the maximum value of $x-1+y$ subject to the constraints
% \begin{align*}
% x,y \ge 0 \\
%   x - 1 + 4y & \le n \\
%   2x + 1 + y & \le n
...
\begin{compactenum}[({DM}1)]
    ...
    \item each edge $vw$ on the boundary of the outer face of $H$ is also on the boundary some inner face $vwx$ of $H$, where $x\in I(H)$. \label[dm]{bad_edge}
...
\begin{figure}
  ...
  \caption{Remove an inner-degree $2$ vertex $v$ is immediately followed by removing an inner-degree $3$ vertex $w$.}
  \label{2_3_figure}
\end{figure}
...
\begin{proof}
  ...
  Let $x$ be the unique neighbour of $w$ in $H-B(H)$. Since $x$ and $w$ are vertices of $H-B(X)$, $x,w\in I(H)$.  Since $w$ is an inner vertex in a near-triangulation, it is incident to $t\ge 3$ faces $v_iv_{i+1}w$ for $i\in\{0,\ldots,v_{t}\}$, with $v_0=v_t=x$.  Since $\deg_{H-B(H)}(w)=1$, $v_1,\ldots,v_{t-1}\in B(H)$.  Therefore, $H$ contains no edge $v_i v_{i+r}$ for any $i\in\{0,\ldots,t-r\}$ and any $r\ge 2$.  Therefore, for each $i\in\{1,\ldots,t-1\}$, the only two inner faces of $H$ that include $v_i$ are $v_{i-1}v_iw$ and $v_iv_{i+1}w$.  Therefore $N^+_H(v_i)=\{w\}$ for each $i\in\{2,\ldots,t-2\}$ and $N^+_H(v_1)=N^+_H(v_{t-1})=\{x,w\}$.\footnote{In fact, \cref{inner_degree_1} implies that $t=3$, but this is not important for this proof.}
...
At last, the following lemma, illustrated in \cref{two_critical_colouring}, shows how we combine everything to find three sets whose total size is at most $2|B(H)| + I(H)$.
...
The proof of \cref{three_sets_coverage} considers each component of $H-B(H)$ separately.  Even wheels are handled directly and other components are handled using \cref{critical}.  The full proof is included in the appendix.
...
The proof of \cref{better_greedy} is similar to that of \cref{simple_greedy} in that it involves defining a small linear program that bounds the worst-case size of $X_r$ is deferred to the appendix.
...
\begin{compactenum}
  ...
  \item Is it true that every $4$-connected $n$-vertex triangulation has a connected dominating set of size at most $n/4+O(1)$?  If this question has a positive answer, then the $O(1)$ term is necessary.  It is easy to construct a $4$-connected $6$-vertex triangulation $G$ for which any connected dominating set has at least two vertices.
...
\begin{proof}[Proof of \cref{two_three_pair}]
  ...
  \begin{figure}
    ...
    \end{tabular}
    \caption{The proof of \cref{really_good}. \hussein{I think, it might be better if we draw another neighbor of w to its right.}}
  ...
  Since $f$ is a face of $H[B(H)]$, the only neighbours of $v_j$ in $B(H)$ are $v_{j-1}$ and $v_{j+1}$. Since $H$ is a near-triangulation, this implies that $w v_j z$ is a face of $H$.  In particular $wz$ is an edge of $H$.
